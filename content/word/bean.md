---
robots: noindex
title: "【bean】の意味・使い方・例文 ― seedとの違い【TOEIC英単語】"
description: "beanの意味・使い方・TOEICでの出題傾向をやさしく解説。例文・クイズ付きでseedとの違いもわかりやすく学べます。"
word: "bean"
part_of_speech: "名詞"
difficulty: "1"
tags:
  - 名詞
  - Part1
  - 中立的
  - 観光
  - 日常会話
compared_word: "seed"
related_words:
  - "pea"
  - "grain"
next_words:
  - "fruit"
  - "vegetable"
last_update: "2025-05-04"  # YYYY-MM-DD
---

## 🔰 beanの発音と意味

<button class="play-audio" onclick="playTTS('bean')">
  <span class="play-audio-main">
    ▶️ 発音を再生　/biːn/
  </span>
  <span class="play-audio-sub">
    （ビーン）
  </span>
</button>

[名詞] 豆（可算）

**bean** は、インゲン豆や大豆など、さやに入った食用の豆やコーヒー豆などを指す単語です。

語源は古英語「bēan」から来ており、古くからヨーロッパで豆類を指す言葉として使われてきました。

他の品詞：  
- beany：豆のような（形容詞）
- beaned：豆をぶつけられた（動詞）

---

## ✏️ beanの例文

【日常会話】  
I like to eat beans with rice.  
　私はご飯と一緒に豆を食べるのが好きです。

【ビジネス英語】  
The coffee beans are imported from Brazil.  
　コーヒー豆はブラジルから輸入されています。

---

## 🎯 TOEICの出題頻度と傾向

- 難易度：★☆☆☆☆
- 出題パート：Part 1（写真描写）

TOEICでは主にPart 1の写真描写で、食材や市場の場面などで登場することがあります。日常的な単語なので難易度は低めです。

---

## ❓ クイズに挑戦！

正しい使い方はどっち？

1. I bought some beans at the market.  
2. I bought some beans at the station.

---

## ✅ クイズの答えと解説

- 正解：**1**
- 解説：豆は市場（market）で買うのが自然です。駅（station）は文脈的に不自然です。

少し迷ったあなたも、  
カンタンだったあなたも、  
理解したら即実践👇️

---

## 🚀 今が英語力を伸ばすチャンス！ (PR)

<div class="info-center">
＼次のステージへ進もう／<br>  
<a href="https://px.a8.net/svt/ejp?a8mat=4556RW+FUYPWY+3AQG+C6QR6" class="ad-link" data-cvid="aid23_bid07" data-difficulty="1" rel="nofollow">TOEICテスト対策</a>
<img border="0" width="1" height="1" src="https://www12.a8.net/0.gif?a8mat=4556RW+FUYPWY+3AQG+C6QR6" alt=""><br>
<a href="https://px.a8.net/svt/ejp?a8mat=4556RW+FUYPWY+3AQG+C7LM9" class="ad-link" data-cvid="aid23_bid07" data-difficulty="1" rel="nofollow">
<img border="0" width="320" height="50" alt="" src="https://www27.a8.net/svt/bgt?aid=250504844959&wid=001&eno=01&mid=s00000015388002051000&mc=1"></a>
<img border="0" width="1" height="1" src="https://www14.a8.net/0.gif?a8mat=4556RW+FUYPWY+3AQG+C7LM9" alt="">
</div>

---

## 🤔  seed との違いは？

- **bean**：食用の豆やコーヒー豆など、さやに入った比較的大きめの豆を指す。
- **[seed](/seed)**：植物の種全般を指し、豆以外の小さな種も含む。

beanは主に食用や特定の植物の大きな種を指し、seedは植物全般の種を広く指します。

---

## 🧩 あわせて覚えたい

- [pea](/pea)：エンドウ豆
- [grain](/grain)：穀物、粒

---

## 📖 できればこれも

- [fruit](/fruit)：果物
- [vegetable](/vegetable)：野菜

<!-- cvid: aid23_bid07 -->
