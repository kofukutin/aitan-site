---
robots: index
title: "【day】の意味・使い方・例文 ― dateとの違い【TOEIC英単語】"
description: "dayの意味・使い方・TOEICでの出題傾向をやさしく解説。例文・クイズ付きでdateとの違いもわかりやすく学べます。"
word: "day"
part_of_speech: "名詞"
difficulty: "1"
tags:
  - 名詞
  - Part1
  - 中立的
  - ビジネス
  - 日常会話
compared_word: "date"
related_words:
  - "week"
  - "night"
next_words:
  - "month"
  - "time"
last_update: "2025-05-04"  # YYYY-MM-DD
---

## 🔰 dayの発音と意味

<button class="play-audio" onclick="playTTS('day')">
  <span class="play-audio-main">
    ▶️ 発音を再生　/deɪ/
  </span>
  <span class="play-audio-sub">
    （デイ）
  </span>
</button>

[名詞] 日、1日、昼間

**day** は、「日」「1日」「昼間」など、24時間の単位や、太陽が出ている時間帯を表す基本的な単語です。

語源は古英語の「dæg」で、太陽が昇っている時間を指していました。現代英語でも「日」や「昼間」の意味で広く使われています。

他の品詞：  
- daily：毎日の（形容詞）
- daily：毎日（副詞）

---

## ✏️ dayの例文

【日常会話】  
Today is a beautiful day.  
　今日は素晴らしい日だ。

【ビジネス英語】  
The meeting is scheduled for the next day.  
　会議は翌日に予定されています。

---

## 🎯 TOEICの出題頻度と傾向

- 難易度：★☆☆☆☆
- 出題パート：Part 1（写真描写）

TOEICでは日付やスケジュール、日常の出来事を表す場面で頻出します。特にPart 1やPart 2でよく見かけます。

---

## ❓ クイズに挑戦！

正しい使い方はどっち？

1. I will finish the report in one day.  
2. I will finish the report in one date.

---

## ✅ クイズの答えと解説

- 正解：**1**
- 解説：「1日で」という意味ではdayを使います。dateは「日付」や「デート」の意味です。

少し迷ったあなたも、  
カンタンだったあなたも、  
理解したら一歩前へ👇️

---

## 🚀 今が英語力を伸ばすチャンス！ (PR)

<div class="info-center">
＼英語が楽しくなる工夫／<br>  
（おすすめ情報予定）
</div>

---

## 🤔  date との違いは？

- **day**：24時間の「日」や「昼間」を表す一般的な単語
- **[date](/word/date/)**：特定の日付やカレンダー上の「日」、または「デート」の意味

dayは時間の長さや日常の「日」を指し、dateはカレンダー上の特定の日やイベントの日付を指します。

---

## 🧩 あわせて覚えたい

- [week](/word/week/)：週、7日間
- [night](/word/night/)：夜、夜間

---

## 📖 できればこれも

- [month](/word/month/)：月、1か月
- [time](/word/time/)：時間、時刻

<!-- cvid: aid40_bid25 -->
