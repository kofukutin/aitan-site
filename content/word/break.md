---
robots: noindex
title: "【break】の意味・使い方・例文 ― pauseとの違い【TOEIC英単語】"
description: "breakの意味・使い方・TOEICでの出題傾向をやさしく解説。例文・クイズ付きでpauseとの違いもわかりやすく学べます。"
word: "break"
part_of_speech: "動詞"
difficulty: "2"
tags:
  - 動詞
  - Part3
  - カジュアル
  - ビジネス
  - 会議
compared_word: "pause"
related_words:
  - "stop"
  - "interrupt"
next_words:
  - "fix"
  - "open"
last_update: "2025-05-04"
---

## 🔰 breakの発音と意味

<button class="play-audio" onclick="playTTS('break')">
  <span class="play-audio-main">
    ▶️ 発音を再生　/breɪk/
  </span>
  <span class="play-audio-sub">
    （ブレイク）
  </span>
</button>

[動詞] 壊す、壊れる、（中断して）休憩する

**break** は、「物を壊す」「中断する」「休憩する」など、物理的・抽象的な"切れ目"や"壊す"動作を表す動詞です。

語源は古英語の "brecan"（壊す）に由来し、「何かを分ける・断つ」というイメージが基本にあります。

他の品詞：  
- break：休憩（名詞）
- breakable：壊れやすい（形容詞）

---

## ✏️ breakの例文

【日常会話】  
Let's take a break.  
　休憩しよう。

【ビジネス英語】  
He broke the contract by leaving early.  
　彼は早く退職して契約を破りました。

---

## 🎯 TOEICの出題頻度と傾向

- 難易度：★★☆☆☆
- 出題パート：Part 3（会話問題）

TOEICでは「休憩する」「壊す」「中断する」など複数の意味で出題されます。会話や説明文で頻出です。

---

## ❓ クイズに挑戦！

正しい使い方はどっち？

1. Let's break for lunch.  
2. Let's break lunch.

---

## ✅ クイズの答えと解説

- 正解：**1**
- 解説：「break for lunch」で「昼食のために休憩する」という意味になります。

少し迷ったあなたも、  
カンタンだったあなたも、  
理解したら即実践👇️

---

## 🚀 今が英語力を伸ばすチャンス！ (PR)

<div class="info-center">
＼試す価値アリの方法／<br>  
<a href="https://px.a8.net/svt/ejp?a8mat=4556RW+FUYPWY+3AQG+BWVTE" class="ad-link" data-cvid="aid23_bid19" data-difficulty="2" rel="nofollow">スタディサプリ ENGLISH</a>
<img border="0" width="1" height="1" src="https://www17.a8.net/0.gif?a8mat=4556RW+FUYPWY+3AQG+BWVTE" alt=""><br>
<a href="https://px.a8.net/svt/ejp?a8mat=4556RW+FUYPWY+3AQG+C7LM9" class="ad-link" data-cvid="aid23_bid19" data-difficulty="2" rel="nofollow">
<img border="0" width="320" height="50" alt="" src="https://www27.a8.net/svt/bgt?aid=250504844959&wid=001&eno=01&mid=s00000015388002051000&mc=1"></a>
<img border="0" width="1" height="1" src="https://www14.a8.net/0.gif?a8mat=4556RW+FUYPWY+3AQG+C7LM9" alt="">
</div>

---

## 🤔  pause との違いは？

- **break**：一時的に中断したり、休憩したり、物を壊す意味もある
- **[pause](/pause)**：一時的に止める、特に動作や話を短く止める意味

breakは「休憩」や「壊す」など幅広い意味で使われ、pauseは「一時停止」に特化しています。

---

## 🧩 あわせて覚えたい

- [stop](/stop)：止める、やめる
- [interrupt](/interrupt)：中断する、邪魔をする

---

## 📖 できればこれも

- [fix](/fix)：修理する、直す
- [open](/open)：開ける

<!-- cvid: aid23_bid19 -->
